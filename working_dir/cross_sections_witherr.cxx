...
void Characterisation ( TH2D *h_smeared, TNtuple *info, const char pr_path[1024] );
...
void cross_sections_witherr( const int &n_protons, const char pr_path[1024] ) {
    ...
    TNtuple *nt_train = new TNtuple("nt_train","Important information about signal","ev:cth:T:neu:cc:nc:np:npip:npim:npi0:qel:res:mec");
    nt_train->SetDirectory(0);

    GetTruth( gst_train, nt_train, 0, n_train, n_protons, truth_T_train, truth_cos_train, truth_detectable_train, impur_T_train, impur_cos_train );
    ...
    // Initiate the TNtuple to hold the interesting features
    TNtuple *nt = new TNtuple("nt","Important information about signal","ev:cth:T:neu:cc:nc:np:npip:npim:npi0:qel:res:mec");
    nt->SetDirectory(0);

    GetTruth( gst_test, nt, 0, n_full, n_protons, truth_T_test, truth_cos_test, truth_detectable_test, impur_T_test, impur_cos_test );
   
    TFile nt_file("nt_file.root", "RECREATE");
    nt->Write("nt");
    ...
    Characterisation( h_true_test, nt, pr_path );
    ...
    // WRITE TO FILE
    TFile ffile("working_dir/flux_selection_ddxsec.root", "UPDATE"); 
...
void GetTruth( TTree *tree, 
    ...
    TBranch *b_ev    = tree->GetBranch("iev");
    ...
    for ( int i = n_min_bins; i < n_max_bins; ++i ){
        ...
        int ev    = b_ev->GetLeaf("iev")->GetValue();
        ...
        if ( ( n_protons != -1 && nfp == n_protons ) || ( n_protons == -1 ) ){
            ...
            if ( cc == 1 && nfpip + nfpim + nfpi0 == 0 ){
                ...
                if ( fspl == 13 ){
                    ...
                    if ( isDetectable ){
                        nt->Fill(ev, cos_mu, T_mu, neu, cc, nc, nfp, nfpip, nfpim, nfpi0, qel, res, mec); 
                    }
                }
            }
...
void Characterisation ( TH2D *h_smeared, TNtuple *info, const char pr_path[1024] ){
    ...
    for ( int i = 1; i <= y_bins; ++i ){
        ...
        conv << setprecision(4) << "working_dir/categorisation/" << pr_path << "/" << pr_path << "_pre_FSI_Tmu_slice_" << low_edge_T << "_" << up_edge_T << ".png";
        title = conv.str();
        ...
        hist << setprecision(4) << "T_{#mu} in [" << low_edge_T << "," << up_edge_T << "], GeV " << pr_path;
        temp = hist.str();
